  ...
  return (
    <section className="or-hero">
      <div style={{ maxWidth: '760px' }}>
        <p style={{ color: '#0A1128', fontSize: '15px', letterSpacing: '0.22em', margin: '0 0 24px 0' }}>
          {p.eyebrow}
        </p>
        ...
        <p style={{ color: 'rgba(10,17,40,0.7)', fontSize: '18px', lineHeight: 1.8, maxWidth: '600px', margin: 0 }}>
          {p.body}
        </p>
      </div>
    </section>
    ...
    <section className="or-narrative">
      <div className="or-narrative-inner">
        ...
        {p.section1.paragraphs.map((para, i) => (
          <p key={i} style={{ fontSize: '18px', color: '#4A4A4A', lineHeight: 1.85, margin: i < p.section1.paragraphs.length - 1 ? '0 0 20px 0' : '0 0 64px 0' }}>
            {para}
          </p>
        ))}
        ...
        {p.section2.paragraphs.map((para, i) => (
          <p key={i} style={{ fontSize: '18px', color: '#4A4A4A', lineHeight: 1.85, margin: i < p.section2.paragraphs.length - 1 ? '0 0 20px 0' : '0' }}>
            {para}
          </p>
        ))}
      </div>
    ...
    <section className="or-vm-section">
      <div className="or-vm-grid">
        <div className="or-vm-block">
          <p style={{ color: '#D4AF37', fontSize: '15px', letterSpacing: '0.2em', margin: '0 0 12px 0' }}>
            {p.vision.label}
          </p>
          <p style={{ fontSize: '20px', color: '#0A1128', fontFamily: 'Georgia, serif', fontWeight: 500, lineHeight: 1.6, margin: '0 0 16px 0' }}>
            {p.vision.main}
          </p>
          <p style={{ fontSize: '18px', color: '#4A4A4A', lineHeight: 1.8, margin: 0 }}>
            {p.vision.expansion}
          </p>
        </div>
  ...
  return (
    <section className="or-convictions">
      <div className="or-convictions-inner">
        <p style={{ color: '#D4AF37', fontSize: '15px', letterSpacing: '0.22em', margin: '0 0 14px 0' }}>
          {p.eyebrow}
        </p>
        <h2 style={{ color: '#ffffff', fontFamily: 'Georgia, serif', fontSize: '32px', fontWeight: 400, margin: '0 0 20px 0', lineHeight: 1.25 }}>
          {p.heading}
        </h2>
        <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '18px', lineHeight: 1.85, margin: '0 0 40px 0' }}>
          {p.intro}
        </p>
        {p.items.map((item, i) => (
          <div key={i}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '18px', lineHeight: 1.85, margin: 0 }}>
              {item.body}
            </p>
          </div>
        ))}
    ...
    <section className="or-ahead">
      <div className="or-ahead-inner">
        ...
        {p.paragraphs.map((para, i) => (
          <p key={i} style={{ fontSize: '18px', color: '#4A4A4A', lineHeight: 1.85, margin: i < p.paragraphs.length - 1 ? '0 0 20px 0' : '0' }}>
            {para}
          </p>
        ))}
      </div>
  ...
  return (
    <section className="or-cta">
      ...
      <div style={{ display: 'flex', gap: '14px', justifyContent: 'center', flexWrap: 'wrap' }}>
        <button className="or-cta-btn-dark">{p.btn1}</button>
